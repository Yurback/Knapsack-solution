let count = 0;
function knapsackfn(items, cap, cur_item, mem) {
    console.log('running');
    count += 1;
    if(mem[cap][cur_item]) return mem[cap][cur_item];
            //base case
    if ( cap === 0 || cur_item < 0 ) {
    ...
    if ( cap<items[cur_item].weight ) {
        ...
        return knapsackfn(items, cap, cur_item-1, mem);
    }
    const sackWithItem = knapsackfn(items,cap-items[cur_item].weight,cur_item-1,mem);
    const sackWithoutItem = knapsackfn(items, cap, cur_item-1,mem);
...
function knapsack(items, cap, cur_item) {
    ...
    console.log(mem);
    return knapsackfn(items, cap, cur_item, mem);
}
